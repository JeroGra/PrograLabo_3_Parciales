...
/////////////////// CRUD  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//LISTAR
app.get('/listarArticulosBD', verificar_jwt, (request, response) => {
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
...
//AGREGAR
app.post('/agregarArticuloBD', verificar_jwt, upload.single("foto"), (request, response) => {
    let file = request.file;
    let extension = mime.extension(file.mimetype);
    let obj = JSON.parse(request.body.articulo_json);
    let path = file.destination + obj.tipo + "." + extension;
    fs.renameSync(file.path, path);
    obj.path_foto = path.split("public/")[1];
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("insert into articulos set ?", [obj], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }
...
//MODIFICAR
app.post('/artis', verificar_jwt, upload.single("foto"), (request, response) => {
    let file = request.file;
    let extension = mime.extension(file.mimetype);
    let obj = JSON.parse(request.body.articulo);
    let path = file.destination + obj.tipo + "_modificacion" + "." + extension;
    fs.renameSync(file.path, path);
    obj.path_foto = path.split("public/")[1];
    let obj_modif = {};
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("update articulos set ? where id = ?", [obj_modif, obj.id_articulo], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }
...
//ELIMINAR
app.delete('/artis', verificar_jwt, (request, response) => {
    let obj = request.body;
    ...
    request.getConnection((err, conn) => {
        ...
        conn.query("select path_foto from articulos where id = ?", [obj.id_articulo], (err, result) => {
            if (err)
                throw ("Error en consulta de base de datos.");
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("delete from articulos where id = ?", [obj.id_articulo], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }
