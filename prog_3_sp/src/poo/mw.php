...
use ElMuñeco\Juguete;
use ElMuñeco\Usuario;
...
class MW
{
    ...
    public static function ifSet(Request $request, RequestHandler $handler) : Response
    {
    ...
    public static function VerificarPropietario(Request $request, RequestHandler $handler) : Response 
    {
        ...
        $usuario = $objJwt->payload->usuario;
    ...
    public static function ListadoTablaGet(Request $request, RequestHandler $handler) : Response  
    {
    ...
    public static function ListadoUsuariosPropietario(Request $request, RequestHandler $handler) : Response  
    {
        ...
        { 
            $objRt= new stdClass();
            $objRt->mensaje = "error no es propietario";
            $objRt->exito = false;
            $response = new Response(403);
            $response->getBody()->write(json_encode($objRt));
        }

        return $response->withHeader('Content-Type', 'application/json');
    }

    public function IdImparesJuguetes(Request $request, RequestHandler $handler) : Response  
    {

        $response = new Response(200);

        $listado = array();
        $jueguetes = Juguete::traerJuguetes();
    
        foreach($jueguetes as $toys)
        {
            $obj = new stdClass;
            $obj->id = $toys->id;
            $obj->marca = $toys->marca;
            $obj->precio = $toys->precio;
            $obj->path_foto = $toys->path_foto;
